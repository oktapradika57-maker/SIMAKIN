...
<p style="font-size:12px; color:#94a3b8; margin-bottom:15px; line-height:1.5;">Memantau progres jumlah inputan tim valid berdasarkan target unik masing-masing NOP cabang.</p>
        ...
        with tab_trk_asset:
            html_trk_asset = ""
            for branch, target in target_default.items(): html_trk_asset += render_progress_nop(f"NOP {branch.title()}", prog_asset[branch], target)
            st.markdown(html_trk_asset, unsafe_allow_html=True)
            
        with tab_trk_genset:
            html_trk_genset = ""
            for branch, target in target_genset.items(): html_trk_genset += render_progress_nop(f"NOP {branch.title()}", prog_genset[branch], target)
            st.markdown(html_trk_genset, unsafe_allow_html=True)
            
        with tab_trk_tools:
            html_trk_tools = ""
            for branch, target in target_default.items(): html_trk_tools += render_progress_nop(f"NOP {branch.title()}", prog_tools[branch], target)
...
• Pontianak: <b style="color:#ffffff;">31</b>
        ...
        # 🔥 ITEM TAMBAHAN BARU: TABEL STATUS TOOLS TIM (OKE/NY/NOK) SESUAI FILTER JABATAN 🔥
        st.markdown("<br>", unsafe_allow_html=True)
        with st.expander("📋 REKAPITULASI KONDISI TOOLS TIM (OKE / NOK / NY)"):
            st.markdown(f"<p style='font-size:12px; color:#cbd5e1;'>Menampilkan status tools (Agregasi) untuk <b>{len(df_sdm_filtered)} Personel</b> berdasarkan filter Jabatan/Loker di atas.</p>", unsafe_allow_html=True)
            ...
            if not df_tools_asset.empty and not df_sdm_filtered.empty:
                ...
                if name_col_tools:
                    ...
                    for _, row in tools_macro_df.iterrows():
                        ...
                        # Hitung Total Teks OKE/NOK/NY di Baris Excel Personel ini
            ...
            if table_data:
                ...
                st.markdown("""<div style='font-size: 10px; color:#94a3b8; line-height: 1.4; margin-top:5px;'>
                ...
                st.info("Belum ada inputan / deteksi Tools (OKE/NY) untuk grup tim ini.")
