...
def render_progress_nop(label, filled, target):
    if target == 0:
        ...
        if pct == 100:
            ...
            color = "#f59e0b"
            warning = f"<span style='color:#f59e0b; font-size:10px; font-weight:bold;'>⚠️ WARNING: Kurang {target - filled} NOP</span>"
        else:
            color = "#ef4444"
            warning = f"<span style='color:#ef4444; font-size:10px; font-weight:bold;'>🚨 CRITICAL: Kurang {target - filled} NOP</span>"
...
if not df_sdm.empty:
    ...
    with col_ai:
        # 🔥 FITUR BARU: TRACKER KEPATUHAN NOP (MENGGANTIKAN INTISARI SERVER) 🔥
        ...
        def calculate_nop(df, col_idx):
            ...
            if len(df.columns) > col_idx:
                ...
                return int(s[~s.isin(['NAN', 'NONE', '', '-', 'NULL', 'NA'])].count())
            return 0
        ...
        <div class="report-box-premium" style="margin-top: 0; padding: 25px;">
            ...
            <p style="font-size:12px; color:#94a3b8; margin-bottom:20px; line-height:1.5;">Memantau progres input Nomor Operation (NOP) pada database master.</p>
